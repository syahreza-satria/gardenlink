...
const LinkCard: React.FC<LinkCardProps> = ({ title, url, icon: Icon }) => {
  return (
    <a
      ...
      className="flex items-center p-4 w-full bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white rounded-full transition-all duration-300 transform backdrop-blur-xl"
      title={title}
    >
      {Icon && (
